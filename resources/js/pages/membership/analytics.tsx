...
export default function Analytics() {
    ...
    const handlePeriodChange = useCallback((val: string) => {
        ...
        if (selectedMember) params.member_id = selectedMember.id;
        navigate(params);
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                {[
                    { label: 'Total Members', value: String(data.kpi.total_members), desc: 'Registered' },
                    { label: 'Active Today', value: String(data.kpi.active_today), desc: 'Checked in' },
                    { label: "Today's Check-ins", value: String(data.kpi.checkins_today), desc: 'Total entries' },
                    { label: 'Period Check-ins', value: String(data.kpi.total_checkins_period), desc: 'In selected period' },
                    { label: 'Unique Members', value: String(data.kpi.unique_members_period), desc: 'In selected period' },
                    { label: 'Avg Daily', value: String(data.kpi.avg_daily_checkins), desc: 'Check-ins / day' },
                ].map((k) => (
                    <Card key={k.label}>
            ...
            <Section title="Daily Activity" desc="Unique active members per day">
                <ResponsiveContainer width="100%" height={260}>
                    <AreaChart data={data.daily_active}>
                        ...
                        <Area type="monotone" dataKey="active_members" stroke="#6366f1" fill="#6366f1" fillOpacity={0.15} strokeWidth={2} name="Active Members" />
                    </AreaChart>
                </ResponsiveContainer>
            </Section>

            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <Section title="Weekly Check-ins" desc="Total check-ins per week">
                    <ResponsiveContainer width="100%" height={260}>
                ...
                <Section title="Day of Week" desc="Total check-ins by day">
                    <ResponsiveContainer width="100%" height={260}>
            ...
            <Section title="Attendance Heatmap" desc="Check-in density by hour and day of week (last 90 days)">
                <HeatmapChart data={data.heatmap} />
            </Section>

            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <Section title="Peak Hours" desc="Total check-ins by hour of day">
                    <ResponsiveContainer width="100%" height={260}>
                        <BarChart data={data.peak_hours}>
                            ...
                            <YAxis fontSize={12} allowDecimals={false} />
                            <Tooltip />
                            <Bar dataKey="count" fill="#f59e0b" radius={[2, 2, 0, 0]} name="Check-ins" />
                        </BarChart>
                    </ResponsiveContainer>
                </Section>

                <Section title="Monthly Trend (YoY)" desc="Unique visits per month">
                    <ResponsiveContainer width="100%" height={260}>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <Section title="Check-in Methods (Daily)" desc="Daily breakdown by method">
                    <ResponsiveContainer width="100%" height={260}>
                        <BarChart data={data.method_distribution}>
                            ...
                            {Object.keys(METHOD_COLORS).map((m) => (
                                <Bar key={m} dataKey={m} stackId="a" fill={METHOD_COLORS[m]} name={m.replace('_', ' ')} />
                            ))}
                        </BarChart>
                    </ResponsiveContainer>
                </Section>

                <Section title="Method Share" desc="Check-in method proportions">
                    <div className="flex h-[260px] items-center justify-center">
                        {data.method_pie.length > 0 ? (
                            <ResponsiveContainer width="100%" height={260}>
                                <PieChart>
                                    <Pie data={data.method_pie} cx="50%" cy="50%" outerRadius={90} paddingAngle={3}
                                        dataKey="value" label={({ name, percent }) => `${name.replace('_', ' ')} ${(percent * 100).toFixed(0)}%`}>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                <Section title="Member Growth" desc="Cumulative members over time">
                    <ResponsiveContainer width="100%" height={220}>
                        <AreaChart data={data.member_growth}>
                            ...
                            <YAxis fontSize={11} />
                            <Tooltip />
                            <Area type="monotone" dataKey="total_members" stroke="#10b981" fill="#10b981" fillOpacity={0.12} strokeWidth={2} name="Total" />
                        </AreaChart>
                    </ResponsiveContainer>
                </Section>

                <Section title="Status Breakdown" desc="Current member distribution">
                ...
                <Section title="Session Duration" desc="Distribution of visit lengths">
                    <ResponsiveContainer width="100%" height={220}>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <Section title="New vs Returning" desc="First-time vs returning check-ins per day">
                    <ResponsiveContainer width="100%" height={260}>
                        <BarChart data={data.new_vs_returning}>
                            ...
                            <Bar dataKey="returning" stackId="a" fill="#6366f1" name="Returning" />
                        </BarChart>
                    </ResponsiveContainer>
                </Section>

                <Section title="Weekly Retention" desc="% of members returning week over week">
                    <ResponsiveContainer width="100%" height={260}>
                        <LineChart data={data.retention_rate}>
                            ...
                            <Line type="monotone" dataKey="rate" stroke="#10b981" strokeWidth={2} dot={false} name="Retention Rate" />
                        </LineChart>
                    </ResponsiveContainer>
                </Section>
            </div>
...
function MemberAnalytics({ data }: { data: MemberData | null }) {
    if (!data) {
        return <Card><CardContent className="flex flex-col items-center gap-2 py-16">
            <p className="text-muted-foreground text-sm">Select a member above to view their analytics.</p>
        </CardContent></Card>;
    }
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                <Section title="Visit Activity" desc="Check-in volume and recency">
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <p className="text-xs text-muted-foreground">30-Day Visits</p>
                            ...
                            <div>
                                ...
                                <p className="mt-0.5 text-3xl font-bold tracking-tight">{data.kpi.avg_frequency_per_week}<span className="text-base font-normal text-muted-foreground">/wk</span></p>
                            </div>
                        </div>
                        <div className="flex items-center justify-between border-t pt-3">
                            <div>
                                ...
                                <p className="mt-0.5 text-lg font-bold">{data.kpi.this_week_visits}
                                    <span className={`ml-1.5 text-sm font-medium ${weekChange > 0 ? 'text-emerald-600' : weekChange < 0 ? 'text-rose-600' : 'text-muted-foreground'}`}>
                                        {weekChange === 0 ? '— same' : weekChange > 0 ? `+${weekChange}` : weekChange}
                                    </span>
                                </p>
                            </div>
                            <div className="text-right">
                                ...
                                <p className="mt-0.5 text-lg font-bold">
                                    {data.kpi.last_visit_days_ago === null ? 'Never' : data.kpi.last_visit_days_ago === 0 ? 'Today' : data.kpi.last_visit_days_ago === 1 ? 'Yesterday' : `${data.kpi.last_visit_days_ago}d ago`}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <Section title="Engagement" desc="Consistency and loyalty metrics">
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p className="mt-0.5 text-[11px] text-muted-foreground">vs 12-visit benchmark</p>
                            </div>
                            <div>
                                ...
                                <p className="mt-0.5 text-[11px] text-muted-foreground">Days visited in period</p>
                            </div>
                        </div>
                        <div className="flex items-center justify-between border-t pt-3">
                            <div>
                ...
                <Section title="Risk & Behavior" desc="Churn indicators and preferences">
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <div className={`rounded-none px-3 py-1.5 text-xs font-semibold ${
                                ...
                                {data.kpi.churn_risk === 'Low' ? 'Stable' : data.kpi.churn_risk === 'Medium' ? 'Warning' : 'Critical'}
                            </div>
                        </div>
                        <div className="flex items-center justify-between border-t pt-3">
                            <div>
            ...
            <Section title="Attendance Calendar" desc="Daily check-in history (past 12 months)">
                <CalendarHeatmap dates={data.attendance_dates} />
            </Section>

            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <Section title="Weekly Comparison" desc="This week vs last week">
                    <div className="flex h-[200px] items-center justify-center">
                        <ResponsiveContainer width="100%" height={200}>
                            <BarChart data={[
                                { label: 'Last Week', visits: data.weekly_comparison.last_week },
                                { label: 'This Week', visits: data.weekly_comparison.this_week },
                            ]}>
                                <CartesianGrid strokeDasharray="3 3" />
                    ...
                    {data.weekly_comparison.change !== 0 && (
                        <p className="mt-2 text-center text-sm text-muted-foreground">
                            {data.weekly_comparison.change > 0 ? '↑' : '↓'} {Math.abs(data.weekly_comparison.change)}% vs last week
                        </p>
                    )}
                </Section>

                <Section title="Monthly Active Days" desc="Days visited per month">
                    <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={data.monthly_active_days}>
                            ...
                            <YAxis fontSize={11} allowDecimals={false} />
                            <Tooltip />
                            <Bar dataKey="active_days" fill="#10b981" radius={[4, 4, 0, 0]} name="Active Days" />
                        </BarChart>
                    </ResponsiveContainer>
                </Section>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <Section title="Favorite Days &amp; Arrival Time" desc="Visit frequency and avg check-in hour by day">
                    <ResponsiveContainer width="100%" height={260}>
                ...
                <Section title="Visit Hour Distribution" desc="When this member typically checks in">
                    <ResponsiveContainer width="100%" height={260}>
                        <BarChart data={data.visit_hour_distribution}>
                            ...
                            <YAxis fontSize={11} allowDecimals={false} />
                            <Tooltip />
                            <Bar dataKey="count" fill="#f59e0b" radius={[2, 2, 0, 0]} name="Visits" />
                        </BarChart>
                    </ResponsiveContainer>
                </Section>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <Section title="Session Duration Trend" desc="Minutes per visit over time">
                    <ResponsiveContainer width="100%" height={260}>
                        <LineChart data={data.duration_trend}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="date" fontSize={10} tickFormatter={(v: string) => { const d = new Date(v); return `${d.getMonth() + 1}/${d.getDate()}`; }} />
                            <YAxis fontSize={11} label={{ value: 'Minutes', angle: -90, position: 'insideLeft', style: { fontSize: 11 } }} />
                            <Tooltip labelFormatter={(v: string) => new Date(v).toLocaleDateString()} formatter={(v: number) => [`${v} min`, 'Duration']} />
                            <Line type="monotone" dataKey="duration_minutes" stroke="#10b981" strokeWidth={2} dot={false} name="Duration" />
                        </LineChart>
                    </ResponsiveContainer>
                </Section>

                <Section title="Membership Utilization" desc="Visits vs 12-visit benchmark">
                    <div className="flex flex-col items-center justify-center py-4">
                        <GaugeChart value={data.utilization_rate} />
                        <p className="mt-3 text-sm text-muted-foreground">{data.kpi.visits_last_30_days} visits in last 30 days</p>
                        <div className="mt-3 grid grid-cols-2 gap-4 text-center text-sm">
                            <div><p className="text-lg font-bold text-emerald-600">{data.consistency_score}%</p><p className="text-[10px] text-muted-foreground">Consistency</p></div>
                            <div><p className="text-lg font-bold text-amber-600">{data.streak.current}d</p><p className="text-[10px] text-muted-foreground">Current Streak</p></div>
                        </div>
                    </div>
                </Section>
            </div>
...
function CalendarHeatmap({ dates }: { dates: AttendanceDate[] }) {
    ...
    cells.forEach((cell, i) => {
        if (currentWeek.length === 0) {
            for (let j = 0; j < cell.date.getDay(); j++) currentWeek.push({ date: new Date(0), count: -1 });
        }
        currentWeek.push(cell);
        if (cell.date.getDay() === 6 || i === cells.length - 1) { weeks.push(currentWeek); currentWeek = []; }
    });
        ...
        <div className="overflow-x-auto">
            <div className="flex gap-0.5" style={{ minWidth: 600 }}>
                <div className="flex flex-col gap-0.5 pr-1 pt-5">
                    {['', 'Mon', '', 'Wed', '', 'Fri', ''].map((l, i) => (
                        <div key={i} className="text-muted-foreground flex h-3 items-center text-[9px]">{l}</div>
                    ))}
                </div>
                {weeks.map((week, wi) => (
                    <div key={wi} className="flex flex-col gap-0.5">
                        {week.map((cell, ci) => {
                            if (cell.count < 0) return <div key={ci} className="size-3" />;
                            const intensity = cell.count > 0 ? Math.min(cell.count / maxDuration, 1) : 0;
                            return <div key={ci} className="size-3 rounded-[2px]"
                                title={`${cell.date.toLocaleDateString()}: ${cell.count > 0 ? `${cell.count} min` : 'No visit'}`}
...
function GaugeChart({ value }: { value: number }) {
    ...
    return (
        <svg width={200} height={140} viewBox="0 0 200 140">
            ...
            <text x={cx} y={cy + 14} textAnchor="middle" className="fill-muted-foreground" fontSize={10}>Utilization</text>
        </svg>
    );
}

Analytics.layout = { breadcrumbs: [{ title: 'Analytics', href: '/membership/analytics' }] };
